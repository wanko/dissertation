...
class Propagator:
    def __init__(self):
        self.__l2e = {}    # {literal: [(node, node, weight)]}#(\label{prg:dl:propagator:member:l2e}#)
        self.__e2l = {}    # {(node, node, weight): [literal]}#(\label{prg:dl:propagator:member:e2l}#)
        self.__states = [] # [Graph]#(\label{prg:dl:propagator:member:state}#)

    def init(self, init):#(\label{prg:dl:propagator:init:begin}#)
        for atom in init.theory_atoms:#(\label{prg:dl:propagator:init:loop:begin}#)
            term = atom.term
            if term.name == "diff" and len(term.arguments) == 0:
                if len(atom.guard[1].arguments) > 0:#(\label{prg:dl:propagator:init:edge:begin}#)
                ...
                edge = (u, v, weight)#(\label{prg:dl:propagator:init:edge:end}#)
                lit = init.solver_literal(atom.literal)#(\label{prg:dl:propagator:init:map-literal}#)
                self.__l2e.setdefault(lit, []).append(edge)#(\label{prg:dl:propagator:init:l2e}#)
                self.__e2l.setdefault(edge, []).append(lit)#(\label{prg:dl:propagator:init:e2l}#)
                init.add_watch(lit)#(\label{prg:dl:propagator:init:end}\label{prg:dl:propagator:init:loop:end}#)

    def propagate(self, control, changes):#(\label{prg:dl:propagator:propagate:begin}#)
        state = self.__state(control.thread_id)#(\label{prg:dl:propagator:propagate:state}#)
        level = control.assignment.decision_level
        for lit in changes:#(\label{prg:dl:propagator:propagate:loop:begin}#)
            for edge in self.__l2e[lit]:
                cycle = state.add_edge(level, edge)#(\label{prg:dl:propagator:propagate:add-edge}#)
                if cycle is not None:
                    c = [self.__literal(control, e) for e in cycle]
                    control.add_nogood(c) and control.propagate()#(\label{prg:dl:propagator:propagate:add-nogood}#)
                    return#(\label{prg:dl:propagator:propagate:end}#)#(\label{prg:dl:propagator:propagate:loop:end}#)

    def undo(self, thread_id, assign, changes):#(\label{prg:dl:propagator:undo:begin}#)
        self.__state(thread_id).backtrack(assign.decision_level)#(\label{prg:dl:propagator:undo:end}#)
