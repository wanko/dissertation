...
\volumeinfo  %
    {Manuel Carro, Andy King, Marina De Vos, and Neda Saeedloei} %
    {4}  %
...
%
\EventShortName{ICLP'16 TC}
\DOI{10.4230/OASIcs.ICLP.2016.first-page-number}
...
\mysubsection{Solving queries in \asprin}{sec:queries}
%
...
the existence of a stable model of a disjunctive logic program $P$ (see~\cite{roscwa16a}). %
...
Trying to alleviate this problem, algorithm \Qlabel{3} enumerates optimal models of \lm{$P \cup \{$:- not q.$\}$}, 
...
before finding a truly optimal one.
...
from~\cite{basotainsc13a}, \emph{Crossing minimization} from ASP competition 2013, \emph{Metabolic network expansion} from \cite{schthi09a},
\emph{Biological network repair} from \cite{geguivscsithve10a} and \emph{Circuit Diagnosis} from~\cite{sidiqqi11a}.
Since we required instances with multiple optimal solutions, we exclusively focus on Pareto optimality. 
...
We chose for each group the same preference type, either cardinality or subset minimization, and defined a Pareto preference that incorporates all cardinality or the subset preferences respectively.
We calculated optimal solutions of that regarding those Pareto preference statements.
...
For more detailed tables, we refer to~\cite{roscwa16a}. %
...
The second is the minimal distance among all pairs of solutions of a set in percent.
%
...
since it was observed in~\cite{brderosc15b} that this improved the performance of the system.

Table~\ref{tab:time_comparison_small}(a) provides in column T the average runtime and in column TO the sum of timeouts. 
...
In Table~\ref{tab:diverse_comparison_small}(b), for each method we see the score in column S, and 
the average of the diversification quality (over the instances solved by the method) in column avg. 
...
\Alabel{1} has the best quality with a score of 15, followed by \Alabel{2}-dist-to, \Alabel{2}-pg, \Alabel{3}-pg-l and \Alabel{3}-pg-l-rd.
...
\Alabel{2}-pg, \Alabel{3}-pg-l and \Alabel{3}-pg-l-rd are also the first, second and third place, respectively, for average diversification quality. 
Next, with scores ranging from 10-7, we see \Alabel{2} methods 
that do not take into account the whole previous set, 
or that were simply unable to find many solutions at all, as in the case of \Alabel{2}-dist. 
...
Table~\ref{tab:min_dist_comparison_small}(c) 
provides information in analogy to Table~\ref{tab:diverse_comparison_small}(b) for the minimal distance among the solutions. 
%
The overall grouping of the methods is similar to Table~\ref{tab:diverse_comparison_small}(b). 
%
The best methods considering score and average minimal distance, 
viz.\ \Alabel{1}, \Alabel{2}-dist-to, \Alabel{3}-pg-l-rd, \Alabel{3}-pg-l, \Alabel{2}-pg, utilize information from the whole
previous solution set and have strict diversification techniques. 
%

Overall, plain heuristic methods perform better as regards runtime 
while more complex methods, depending on all previous solutions, lead to better quality. 
%
Furthermore, \Alabel{3}-pg-l-rd and \Alabel{3}-pg-l provide the best trade-off between performance and quality. 
%
While \Alabel{1}, \Alabel{2}-dist-to and \Alabel{2}-pg achieve higher quality, they could solve only 18\%, 16\% and 13\% of the instances. 
%
On the other hand, \Alabel{3}-pg-l-rd and \Alabel{3}-pg-l provide good diversification quality and minimal distance while solving 46\% of the instances. 
%
%
...
and an automated approach for the generate and test methodology of~\cite{eitpol06a},
